...
```bash
docker compose exec postgres psql -U postgres -d firmware_hub
```

### Running PostgreSQL Container Manually (without Docker Compose)
If you prefer running the PostgreSQL container manually with `docker run`:
```bash
docker run -d \
...
2. Configure your `.env` file:
   ```env
   DATABASE_URL="postgres://postgres:your_secure_password@localhost:5432/firmware_hub"
   ```
